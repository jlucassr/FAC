...
main:
	...
	#atribui o primeiro valor digitado pelo usuário ao registrador $s2 
	move $s2, $v0 
	
	#Verifica se o primeiro valor digitado pelo usuário é menor que 1 e armazena o resultado no registrador $t1
	slti $t1, $s1, 1	
	#Verifica se o primeiro valor digitado pelo usuário é menor que 1 e armazena o resultado no registrador $t2
	slti $t2, $s2, 1
	...
	#Se o $t5 for verdadeiro vai mostrar mensagem de digito inválido
	#imprime msg
	li   $v0, 4
	la   $a0, msg
	syscall
	li   $v0, 4
	syscall
	#parar de executar o programa se o digto for inválido
	li   $v0, 10
	syscall
